...
def _resolve_path(p, anchors):
    ...
    Even if not found, return a "best guess" for convenient error printing.
    """
    p = Path(str(p))
    if p.is_absolute():
        return p
...
def _fix_lightning_version(path_like):
    """
    If contains .../lightning_logs/version_X/... and that path doesn't exist,
    automatically find the latest version_* under the same lightning_logs and replace.
    """
...
def predict_inr(configs):
    # Make ${case} and other interpolations take effect immediately
    ...
    repo_root = Path(__file__).resolve().parent  # Current systems file directory
    ...
    if getattr(pipeline_configs.inr, "decoder", None) and getattr(pipeline_configs.inr.decoder, "ckpt", None):
        ...
        # Ensure fields exist to avoid subsequent getattr errors
    ...
    # Input dimension: if custom then read custom coordinates; otherwise infer from dataset samples
